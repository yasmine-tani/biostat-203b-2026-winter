...
Read.csv has a run time of 4.428 seconds and utilized 200.10 MB of memory. Produces a data frame.
...
Fread is the fastest function with an elapsed time of 0.259 seconds in comparison to 0.367 and 4.428 seconds. fread and read.csv produce data frames while read_csv produces a tibble with tidyverse.
...
#commented out because rendering was taking forever (as expected in instructions)
```
...
#DUCKDBBBB
system.time({
  duckdb_subset <- arrow::open_dataset("labevents.parquet") %>%
    arrow::to_duckdb() %>%
    dplyr::select(subject_id, itemid, charttime, valuenum) %>%
    dplyr::filter(itemid %in% labitems) %>%
    dplyr::collect()
...
Ok so DuckDB is essentially a database management system that allows you to run code entirely serverless. It is also in-process so it is inside your R/Python/etc. session so you do not need a background program. This is very popular particulary for analytics.
...
```{r}
#vitals we want
chartitems <- c(220045, 220181, 220179, 223761, 220210)
items_string <- paste(chartitems, collapse = "|")

#filter command
cmd <- paste0(
  "zcat < ~/mimic/icu/chartevents.csv.gz | ",
  "awk -F, 'BEGIN {OFS=\",\"} {if (NR==1 || $7 ~ /^(", 
  items_string, 
  ")$/) print $1, $5, $7, $9}' | ",
...
#load into r
chartevents_subset <- read_csv("chartevents_filtered.csv.gz",
  col_types = cols(
    subject_id = col_integer(),
    charttime = col_datetime(),
    itemid = col_integer(),
...
#results
dim(chartevents_subset)
head(chartevents_subset)
```
...
I use Gemini pro because students are awarded one year of pro, and I find that it helps me significantly when I need a concept explained or when I am stuck/have a bug. They certainly improve my productivity.

![](images/clipboard-2160826793.png)

Here, I was stuck on how to set up my models with the IDs connecting to vital signs. It just gave me random numbers associated with vitals that I was not utilizing in the problem:

![](images/clipboard-1643008751.png)

This time, it suspected the position of these variables based off of no prior context on where they were in my data, and it was incorrect.

In all honesty, those were the only errors I had this time around, though I did have more. I ask very specific questions, so I did not see more than 2 significant errors.
